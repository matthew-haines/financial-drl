...
class Environment():
    ...
    def generate_state(self):
        ...
        for i in range(5, 10):  # Normalize with z-score
            mean = torch.sum(self.state[:, 0, i]) / self.state.shape[0]
            sigma = 0
    ...
    def get_account_value(self):
        ...
        return (self.cur_position**2)**(1/2) * self.raw_data[self.cur_step, 7]
    ...
    def step(self, action):
        ...
        reward = torch.log(self.cur_capital / prev_acc_value)
...
class Agent():
    ...
    def update_target(self):
        ...
        for name, param in net_dict.items():
            target_net_dict[name] = (1 - self.tau) * \
                target_net_dict[name] + self.tau * param
        ...
        for state, action, next_state, reward, done in batch:
            ...
            if not done:
                target = reward + self.gamma * \
                    self.target_policy(next_state)[
                        torch.argmax(self.policy(next_state))]
...
# Globals
timesteps = 100
state_space = 10  # OHLCV + TIME ENCODING + POSITION + CAPITAL
action_space = 2
...
for sim in range(n_simulations):
    ...
    for i in range(env.total_steps):
        ...
        if len(agent.memory) > agent.batch_size and i % 96 == 0:
